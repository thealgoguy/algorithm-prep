...
public class FractionalKnapsack {
  public static void main(String args []) {
	  ...
	  int curr_weight=0;
	  for(Item i : arr) {
		  if(curr_weight+i.weight <=W) {  //check if ith item can be taken wholly
...
class Item implements Comparable<Item>{
 ...
 public int compareTo(Item x) {  //sorting in decreasing order of value/weight
	 return Double.compare((double)x.value/x.weight, (double)this.value/this.weight);
 }
}
