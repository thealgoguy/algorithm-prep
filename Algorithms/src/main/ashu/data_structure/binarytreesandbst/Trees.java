 ...
 class BST {
   ...
   public void MorrisPreorderTraversal() {
	   ...
	   while(temp != null) {
		   if(temp.left == null){
			   ...
			   while(pred.right != null && pred.right != temp) pred = pred.right;
			   if(pred.right == null) {
				   System.out.print(temp.data+ " ");
				   pred.right = temp;
				   temp = temp.left;
  ...
  // 3. We just traverse up the tree from the right.
   ...
   public void diameterAndHeight() {
	   ...
	   int d = diameterUtil(this.root, h);
	   System.out.println("Height of BST is : "+h.val);
	   System.out.println("Diameter of BST is : "+d);
   }
   //O(n) time....finding height and diameter separately will be O(n2)
   public int diameterUtil(TreeNode root, Height h) {
	   ...
	   d = Math.max(Math.max(ld, rd), lh.val+rh.val+1); //computing final diameter
	   return d;
   }
   ...
   public TreeNode lowestCommonAncestorBT(TreeNode n1, TreeNode n2) {
	  ...
	  //
	   return lca; //return (f1 && f2) ? lca : null;  //return null if keys not present
   }
...
public class Trees {
	public static void main(String arg []) {
		BST bst = new BST();
		   ...
		   System.out.println("LCA of 3 and 6 is "+bst.lowestCommonAncestorBST(bst.root.right.left, bst.root.right.right));
