...
//operations : extract_max(the root is the min element), insert(at the end),create(inserting n times)
//After every insert, bubbleUp() must be called to preserve order of heap
//After every extract_max, bubbleDown() must be called to preserve heap order. 
...
class MinHeap {
	...
	private void bubbleDown(int root)  {
		...
		if(right==-1) { //only left child node exists
			int min = Math.min(list.get(root), list.get(left));
			if(list.get(root)!=min) {
				swap(root,left);
				bubbleDown(left);
			}
		}
			...
			int min = Math.min(list.get(root), list.get(left));
			min = Math.min(min,list.get(right));
			if(list.get(root)!=min) {
				if(min==list.get(left)) {
					swap(left,root);
					bubbleDown(left);
				}
				else {
...
public class MinHeapImpl {
   public static void main(String args []) {
	   ...
	   else System.out.println("\nSmallest element is "+ minElement);
	   System.out.println("Deleting elements one by one for heapsort(ascending)");
	   for(int i=0; i<a.length; i++) {
		   System.out.print(mh.extractMin()+" ");
		   /*System.out.print("\nDeleted = "+mh.extractMin()+" ");
		   mh.dispay();*/
	   }
   }
}
